...
def main():
    ...
    with a.input.open(encoding='utf-8',newline='') as f:
        r=csv.DictReader(f)
        rows=[x for x in r if x.get('R5PriorityV3') in {'M1_UA_ACTOR_UA_GEO','M2_REMAINS_UNRESOLVED'}]
    if len(rows)!=47:
        raise RuntimeError(f'expected 47 rows, found {len(rows)}')
    ...
    for x in rows:
        out.append({
            ...
            'sources_matching':x.get('sources_matching',''),'source_urls':clean(x.get('source_urls',''),320),
            ...
            'text_excerpt':clean(x.get('representative_text',''),700)
        })

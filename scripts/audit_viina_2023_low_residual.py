...
RU=bounded([
 ...
 r"belgorod",r"bryansk",r"kursk",r"rostov",r"krasnodar",r"voronezh",r"oryol",r"kaluga",r"tula",r"lipetsk",r"tambov",r"ryazan",r"smolensk",r"pskov",r"novgorod",r"saratov",r"samara",r"tatarstan",r"volgograd",r"stavropol",r"moscow",r"tver",r"engels",r"taganrog",r"taman",r"soltsy",r"ilsky",r"afipsky",r"novorossiysk",r"tuapse",
 r"белгород(?:а|е|у|ом)?",r"брянск(?:а|е|у|ом)?",r"курск(?:а|е|у|ом)?",r"ростов(?:а|е|у|ом)?",r"краснодар(?:а|е|у|ом)?",r"воронеж(?:а|е|у|ом)?",r"ор[её]л(?:а|е|у|ом)?",r"калуг(?:а|е|и|ой)",r"тул(?:а|е|ы|ой)",r"липецк(?:а|е|у|ом)?",r"тамбов(?:а|е|у|ом)?",r"рязан(?:ь|и|ью)",r"смоленск(?:а|е|у|ом)?",r"псков(?:а|е|у|ом)?",r"новгород(?:а|е|у|ом)?",r"саратов(?:а|е|у|ом)?",r"самар(?:а|е|ы|ой)",r"татарстан(?:а|е|у|ом)?",r"волгоград(?:а|е|у|ом)?",r"ставропол(?:ь|я|е|ю|ем)?",r"москв(?:а|е|ы|ой|у)",r"твер(?:ь|и|ью)",r"энгельс(?:а|е|у|ом)?",r"таганрог(?:а|е|у|ом)?",r"таман(?:ь|и|ью)",r"сольц(?:ы|ах|ами)?",r"ильск(?:ий|ого|ом)?",r"афипск(?:ий|ого|ом)?",r"новороссийск(?:а|е|у|ом)?",r"туапсе"
])
COMPOSITE=re.compile(r"\|\||;|, а |, и | та | та у | and | while | meanwhile | а в | а у ",re.I|re.U)
YEAR_OTHER=re.compile(r"\b202[24-9]\b|\b2021\b",re.I)
...
def main():
 ...
 if len(rows)!=89: raise RuntimeError(f'Expected 89 low-priority rows, found {len(rows)}')

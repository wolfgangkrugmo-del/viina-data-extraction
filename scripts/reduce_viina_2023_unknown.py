...
# 2023 Russia-proper targets/places that may appear without an oblast/country label.
RU_TARGET=bounded([
 r"ilsky",r"ильск(?:ий|ого|ом)?",r"afipsky",r"афипск(?:ий|ого|ом)?",
 r"volna",r"волна",r"taman",r"таман(?:ь|и|ью|ская|ской)",
 r"soltsy",r"сольц(?:ы|ах|ами)?",r"pskov",r"псков(?:а|е|у|ом)?",
 r"shaykovka",r"шайковк(?:а|е|и|ой)",r"khalino",r"халин(?:о|е|ом)?",
 r"kursk airfield",r"курск(?:ий|ого) аэродром",r"bryansk",r"брянск(?:а|е|у|ом)?",
 r"klintsy",r"клинц(?:ы|ах|ами)?",r"novocherkassk",r"новочеркасск(?:а|е|у|ом)?",
 r"taganrog",r"таганрог(?:а|е|у|ом)?",r"voronezh",r"воронеж(?:а|е|у|ом)?",
 r"ryazan",r"рязан(?:ь|и|ью)",r"kolomna",r"коломн(?:а|е|ы|ой)",
 r"moscow",r"москв(?:а|е|ы|ой|у)",r"odintsovo",r"одинцов(?:о|е|ом)?",
 r"domodedovo",r"домодедов(?:о|е|ом)?",r"ramenskoye",r"раменск(?:ое|ом)?",
 r"novorossiysk",r"новороссийск(?:а|е|у|ом)?",r"tuapse",r"туапсе",
])
...
def main():
 ...
 if len(rows)!=126: raise RuntimeError(f'Expected 126 UNKNOWN rows, found {len(rows)}')
 ...
 for r in rows:
  ...
  if hits: p='H1_RUSSIA_TARGET_TEXT'; reason='KNOWN_RUSSIA_2023_TARGET_OR_PLACE_TEXT'
